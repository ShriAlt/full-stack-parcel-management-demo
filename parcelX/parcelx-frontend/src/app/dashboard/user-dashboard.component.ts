 ...
 <div class="dashboard-wrapper">
  ...
  <main class="main-content">
   ...
   <section *ngIf="activeTab === 'orders'" class="tab-content">
    <h2>My Orders</h2>
    <div class="table-controls"><input class="search-input" placeholder="Search by tracking ID, receiver, status, or location" [(ngModel)]="searchTerm" (ngModelChange)="page = 1" /><select class="search-input sort-select" [(ngModel)]="statusFilter" (ngModelChange)="page = 1"><option value="">All Statuses</option><option value="CREATED">Created</option><option value="IN_TRANSIT">In Transit</option><option value="OUT_FOR_DELIVERY">Out for Delivery</option><option value="DELIVERED">Delivered</option><option value="CANCELLED">Cancelled</option></select><button class="refresh-btn" (click)="loadParcels()">Refresh</button></div>
...
export class UserDashboardComponent implements OnInit {
 ...
 confirmCancel() {
  if (!this.cancelReason.trim()) { this.showPopup('Required', 'Please enter a cancellation reason.'); return; }
  this.http.post(`${environment.apiUrl}/parcels/${this.cancelParcelTarget.id}/cancel`, { parcelId: this.cancelParcelTarget.id, reason: this.cancelReason }).subscribe({ next: () => { this.closeCancel(); this.showPopup('Cancelled', 'Parcel cancellation submitted.'); this.loadParcels(); }, error: (err: any) => this.showPopup('Error', err.error?.message || 'Failed to cancel parcel.') });
 }
 downloadInvoice(parcel: any) {
  const lines = [`INVOICE - parcelX`, `Tracking ID: ${parcel.trackingId}`, `Sender: ${parcel.senderName || parcel.senderUsername}`, `Pickup: ${parcel.pickupAddress} - ${parcel.pickupZipCode}`, `Receiver: ${parcel.receiverName}`, `Drop: ${parcel.dropLocation} - ${parcel.dropZipCode}`, `Weight: ${parcel.weight} g`, `Delivery: ${parcel.deliveryType}`, `Packing: ${parcel.packagingType}`, `Cost: INR ${parcel.cost}`, `Status: ${parcel.status}`, `Pickup Date: ${parcel.pickupDate}`, `Booked: ${new Date(parcel.createdAt).toLocaleString()}`].join('\n');
