...
<div class="dashboard-container">
 <nav class="navbar"><div class="nav-brand">parcelX</div><div class="nav-right"><span>{{ username }}</span><button class="logout-btn" (click)="logout()">Logout</button></div></nav>
...
export class AdminDashboardComponent implements OnInit {
 ...
 ngOnInit() {
  ...
  if (history.state?.loginAcknowledgement) this.showPopup('Welcome to parcelX', `Logged in successfully as ${this.username}.`);
 ...
 validateBookField(field: string) {
  ...
  if (field === 'weight') {
   ...
   if (!/^\d{1,4}$/.test(value || '')) this.bookErrors.weight = 'Weight must contain maximum 4 digits';
